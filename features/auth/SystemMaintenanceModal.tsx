...
import { CsvImportBatch, TestMasterlistPurgePreview, User } from '@/types';
...
const SystemMaintenanceModal: React.FC<Props> = ({ isOpen, onClose, currentUser }) => {
  ...
  useEffect(() => {
    ...
    void api.getImportBatches().then(setBatches).catch((error) => setMessage(error instanceof Error ? error.message : 'Unable to load import batches.'));
  }, [isOpen]);

  const toggleBatch = (id: number) => setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const loadPreview = async () => {
    setBusy(true); setMessage(null);
    try { setPreview(await api.previewTestMasterlistPurge(selected)); } catch (error) { setMessage(error instanceof Error ? error.message : 'Preview failed.'); } finally { setBusy(false); }
  };
  const classify = async () => {
    ...
    try {
      ...
      const result = await api.classifyTestImportBatches({ batchIds: selected, reason, authorizedBy: currentUser.name, authorizedRole: currentUser.role, approvalReference });
      setMessage(`${result.classifiedCount} batch(es) classified as test data. Run validation again before purge.`);
      setBatches(await api.getImportBatches());
  ...
  const purge = async () => {
    ...
    try {
      const valid = await api.verifyPassword(currentUser.username, password);
      if (!valid) throw new Error('Password authorization failed.');
      const result = await api.purgeTestMasterlist({ batchIds: selected, confirmation, reason, authorizedBy: currentUser.name, authorizedRole: currentUser.role, approvalReference });
  ...
  return <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="max-w-2xl">
      ...
      <div className="space-y-4 text-sm">
        ...
        <label className="block font-semibold">Approved import batches</label>
        <div className="max-h-32 overflow-auto rounded border p-2 space-y-1">{batches.map((batch) => <label key={batch.id} className="flex gap-2"><input type="checkbox" checked={selected.includes(Number(batch.id))} onChange={() => toggleBatch(Number(batch.id))} />Batch #{batch.id} — {batch.filename} — {batch.totalRows} rows{(batch as CsvImportBatch & { isTestData?: boolean }).isTestData ? ' — TEST' : ''}</label>)}</div>
        <div className="flex gap-2"><Button type="button" variant="outline" onClick={classify} disabled={busy || selected.length === 0}>Classify selected as test</Button><Button type="button" variant="outline" onClick={loadPreview} disabled={busy || selected.length === 0}>Validate purge scope</Button></div>

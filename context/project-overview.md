# Project Overview — LGU Treasury Connect (Santa Rosa Delinquency Verification & Statement System)

## Overview

LGU Treasury Connect is a mission-critical Real Property Tax Delinquency Verification & Statement System engineered specifically for the Municipal Treasurer's Office of Santa Rosa, Nueva Ecija. It administers official property assessment rolls across Santa Rosa's 33 barangays, calculates statutory tax liabilities, delinquency surcharges, and prompt discounts under **Republic Act No. 7160 (Local Government Code of 1991)**, records external settlement evidence without system cash collection, and generates auditable statements and statutory notices.
...
4. **Arrears-First Verification**: Require chronological review and verification of delinquent periods.
...
1. **Authentication**: Admin or Assessor signs in through Supabase Auth; trusted session claims and RLS authorize access.
...
4. **Scope Selection**: Staff selects delinquent periods under the chronological "Arrears-First" verification rule.
5. **Verification / External Evidence**:
   - Assessor records a verification status and required external settlement reference when evidence exists; no system cash receipt is created.
   - Authorized Admin users may reverse or supersede decisions with mandatory audit justification.
6. **Audit & Reporting**: The system immutably logs all actions to `rptar_audit_logs` and provides printable Statements of Account and statutory notices.
...
- Verification status tracking (`UNVERIFIED`, `VERIFIED_OUTSTANDING`, `VERIFIED_SETTLED_EXTERNALLY`, `DISPUTED`, `NOT_APPLICABLE`, `SUPERSEDED`).
- External settlement evidence references without system cash collection.
- Authorized supervisory reversal and superseding protocol.

### Security, Audit & Administration
- Role-Based Access Control (Admin, Assessor). `SystemMaintenance`, if retained, is non-production/service-only.
...
- Arrears-first delinquency-period verification and external settlement evidence.
- Assessor valuation overrides with mandatory supervisory justification.
- Masterlist bulk CSV import and export with durable review.
- Statement of Account and RA 7160 Section 254 Notice generation.
